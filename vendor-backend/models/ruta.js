...
const Update=async(body)=>{
    let pool= await sql.connect(config)
    const {id,name,adress,contact,propietario,contactoPropietario,limiteCretido,lat,long}=body;
...
const Create=async(body)=>{
    let pool= await sql.connect(config)
    const {name,adress,contact,propietario,contactoPropietario,limiteCretido,lat,long}=body;
    let resilt= await pool.request()
        .input('name',sql.VarChar(50),name)
        .input('adress',sql.VarChar(50),adress)
        .input('contact',sql.VarChar(50),contact)
